...
static char *pack(const some_struct *s, int num, size_t *size);
...
int ex1(void) {
    ...
    printf("pack %zd struct(s): %d byte(s)\n", sizeof(ary)/sizeof(ary[0]), (int)size);
    
    /** UNPACK */
    int num;
    some_struct *s = unpack(buf, (int)size, &num);
...
static char *pack(const some_struct *s, int num, size_t *size) {
    ...
    for (int i = 0; i < num; ++i) {
        msgpack_pack_uint64(&pck, s[i].a);
        msgpack_pack_uint64(&pck, s[i].b);
        msgpack_pack_double(&pck, s[i].c);
    }
    *size = sbuf.size;
